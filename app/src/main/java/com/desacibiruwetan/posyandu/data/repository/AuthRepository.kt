...
class AuthRepository(private val apiService: ApiService) {
    ...
    fun login(request: LoginRequest): Flow<UiState<BaseResponse<LoginData>>> = flow {
    ...
    fun register(request: RegisterRequest): Flow<UiState<BaseResponse<Any>>> = flow {
        emit(UiState.Loading)
    ...
    fun getMe(token: String): Flow<UiState<BaseResponse<UserSchema>>> = flow {
        emit(UiState.Loading)
        try {
    ...
    fun logout(token: String): Flow<UiState<BaseResponse<Any>>> = flow {
        emit(UiState.Loading)
    ...
    private fun parseError(response: Response<*>, defaultMessage: String = "Terjadi kesalahan"): String {
        ...
        return try {
            ...
            val firstErrorMessage = errors?.values?.asSequence()
                ?.filterIsInstance<List<*>>()
                ?.firstOrNull()?.firstOrNull()?.toString()
